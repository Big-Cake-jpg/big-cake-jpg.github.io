...
<template>
  <div v-for="(group, index) in groupedPosts" :key="index">
    <div class="text-3xl" :class="index === 0 ? 'mb-3' : 'my-3'">{{ group[0] }}</div>
    <ul class="space-y-4">
      <li v-for="(post, index) in group[1]" :key="post.path" :class="{ 'mb-0': index === group[1].length - 1 }">
        <RouterLink v-if="post.path" :to="post.path">
          <div class="card rounded-md">
            <div class="media">
              <div class="media-content p-2 m-2">
                <p class="font-semibold">{{ post.title }}</p>
                <p class="text-sm truncate">{{ formatDate(String(post.date)) }}</p>
              </div>
            </div>
          </div>
        </RouterLink>
